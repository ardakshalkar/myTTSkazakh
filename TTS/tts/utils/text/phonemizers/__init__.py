...
from TTS.tts.utils.text.phonemizers.kk_phonemizer import KazakhPhonemizer 
...
PHONEMIZERS = {b.name(): b for b in (ESpeak, Gruut, KO_KR_Phonemizer, BN_Phonemizer,KazakhPhonemizer)} 
...
DEF_LANG_TO_PHONEMIZER["kk"] = KazakhPhonemizer.name() 
...
def get_phonemizer_by_name(name: str, **kwargs) -> BasePhonemizer: 
    ...
    if name == "ja_jp_phonemizer": 
        if JA_JP_Phonemizer is None: 
            raise ValueError(" ❗️ You need to install JA phonemizer dependencies. Try pip install TTS[ja].") 
    ...
    if name =="kazakh_phonemizer": 
        return KazakhPhonemizer(**kwargs) 
